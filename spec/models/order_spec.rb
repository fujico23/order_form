...
RSpec.describe Order, type: :model do
  describe '#total_price' do
    ...
    it { is_expected.to eq 770 }

    context '消費税に端数がでた場合' do
      before do
  ...
  describe '#valid?' do
    ...
    let(:delivery_address) { '新潟県新潟市' }
    let(:payment_method_id) { 1 }
    let(:other_comment) { 'test' }
    ...
    context 'メールアドレスの書式が正しくない場合' do
      let(:email) { 'sampletest' }

      it{ is_expected.to eq false }
    end

    context 'メールアドレスの書式が全角の場合' do
    ...
    context '電話番号が11桁以上入力されている場合' do
      let(:telephone) { '0901234567891' }
    ...
    context 'その他・ご要望が1,000文字の場合' do
      let(:other_comment) { 'a' * 1000 }

      it { is_expected.to eq true }
    end

    context 'その他・ご要望が1,000文字以上の場合' do
      let(:other_comment) { 'a' * 1001 }

      it { is_expected.to eq false }
    end

    context 'ダイレクトメールの配信要否が未選択だった場合' do
